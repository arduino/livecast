class PIXEL_MATRIX {
  ...
  public PALETTE palette = new PALETTE();    // palette with the colors
  ...
  // update the palette using one external
  void initPalette(PALETTE _p) {
    for (int i = 0; i < 32; i++) {
      palette.pix[i].c = _p.pix[i].c;
    }
  }
  ...
  String generateCode(int oM) {
    String code = "";
    int flagInvert = (oM & 0x01);
    int flagCompress = (oM >> 1 & 0x01);

    // check flags
    if ( flagInvert == 1) 
      println("inverting every other line at output");
    if ( flagCompress == 1) 
      println("compressing image using palette");

    if (flagCompress == 0) {
      code = generateFullArrays(flagInvert);
    } else {
      code = generateCompressedArray(flagInvert);
    }
    return code;
  }

  String generateCompressedArray(int oM) {
    String code = "byte ledPixels[] = {\n";
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        if (i%2 == 1 && oM == 1) {
//          int val = 0;
//          val += int(map(red(pix[i*columns + columns -1 - j].c), 0, 255, 0, 15)) << 4;
//          val += int(map(green(pix[i*columns + columns -1 - j].c), 0, 255, 0, 3)) << 2;
//          val += int(map(blue(pix[i*columns + columns -1 - j].c), 0, 255, 0, 3));
            int val = palette.indexOfPalette(pix[i*columns + columns -1 - j].c);
          code += val;
        }
        else {
//          int val = 0;
//          val += int(map(red(pix[i*columns+j].c), 0, 255, 0, 15)) << 4;
//          val += int(map(green(pix[i*columns+j].c), 0, 255, 0, 3)) << 2;
//          val += int(map(blue(pix[i*columns+j].c), 0, 255, 0, 3));
            int val = palette.indexOfPalette(pix[i*columns+j].c);
          code += val;
        }
        if (i*rows + j == columns*rows - 1) 
          code += "};\n";
        else
          if ((i*rows + j + 1)%columns == 0)
            code += ",\n";
          else
            code += ", ";
      }
  ...
  String generateFullArrays(int oM) {
    String code = "byte red[] = {\n";
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
        if (i%2 == 1 && oM == 1)
